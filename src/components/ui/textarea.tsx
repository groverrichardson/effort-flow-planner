...
export interface TextareaProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onChange' | 'onKeyDown'> {
  id?: string;
  value: string;
  ...
  onTiptapKeyDown?: (view: EditorView, event: KeyboardEvent) => boolean | void; // Renamed and type adjusted
...
const Textarea = React.forwardRef<
  ...
  TextareaProps
>(({ id, value, onChange, className, placeholder, showToolbar = true, autoFocus = false, 'data-testid': dataTestId, 'aria-label': ariaLabel, onTiptapKeyDown, ...props }, ref) => {
  const editor = useEditor({
    ...
    editorProps: {
      handleKeyDown: (view: EditorView, event: KeyboardEvent): boolean | void => {
        ...
        if (event.key === ' ') {
          console.log('Tiptap editorProps.handleKeyDown - Spacebar pressed:', {
            key: event.key,
            code: event.code,
          ...
          Promise.resolve().then(() => {
            console.log('Tiptap editorProps.handleKeyDown - Spacebar (after microtask) defaultPrevented:', event.defaultPrevented);
          });
        }
  ...
  React.useEffect(() => {
    if (editor) {
      ...
      if (value !== editorText) {
        try {
          console.log(`Textarea useEffect: value prop ("${value}") !== editor.getText() ("${editorText}"). Calling setContent.`);
          editor.commands.setContent(value, false); // `false` to avoid triggering onUpdate again
        } catch (e) {
          console.error("Textarea: Error in useEffect during setContent:", e);
        }
      } else {
  ...
  return (
    <div ref={ref} className={cn('tiptap-editor-container flex flex-col', className?.includes('min-h-') ? '' : 'min-h-[120px]')}>
      {showToolbar && <EditorToolbar editor={editor} id={toolbarId} />}
