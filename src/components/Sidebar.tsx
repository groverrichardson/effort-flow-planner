...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  console.log(
    "[Sidebar Props Check] isMobileSheetView:",
    isMobileSheetView,
    "isOpen:",
    isOpen,
  );
  console.log(
    "[Sidebar Props Check] allTasks received:",
    allTasks ? allTasks.length : "undefined/empty",
    JSON.stringify(allTasks?.map((t) => t.id)),
  ); // ADDED LOG
          ...
          <div className="space-y-4 mb-6 pb-4 border-b border-gray-300 dark:border-slate-700">
            ...
            <div className="relative">
              ...
              <Input
                ...
                onChange={(e) => {
                  if (!isMobileSheetView) {
                    // Only allow direct typing if not in mobile sheet view
                    onSearchTermChange(e.target.value);
                  }
                }}
                onClick={() => {
                  if (isMobileSheetView) {
                    // Open modal on click if in mobile sheet view
            ...
            <Button
              ...
              size="sm"
              variant="outline"
              className="w-full justify-start gap-1 bg-slate-50 text-gray-800 hover:bg-slate-100"
              id="sidebar-new-note-button"
            >
              <PlusCircle className="h-4 w-4 text-gray-700" />{" "}
              <span className="text-gray-800">New Note</span>
            </Button>
