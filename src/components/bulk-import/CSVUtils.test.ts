import {
    parseRawCSV,
    createInitialColumnMap,
    generateTasksFromCSV,
} from './CSVUtils';
        ...
        it('should correctly map all new fields', () => {
            ...
            expect(tasks[0]).toEqual(
                expect.objectContaining({
                    ...
                    targetDeadline: '2023-03-01', // Both fields set during transition
                    status: 'pending',
                })
            );
            ...
            expect(tasks[1]).toEqual(
                expect.objectContaining({
                    ...
                    completed: false, // 'no'
                    // completedDate, goLiveDate, scheduledDate and targetDeadline are unexpectedly absent for tasks[1]
            ...
            expect(tasks[2]).toEqual(
                expect.objectContaining({
                    ...
                    targetDeadline: '2023-03-15', // Both fields set during transition
                    status: 'pending',
                })
            );
        });
    ...
    describe('createInitialColumnMap', () => {
        it('should correctly map known headers and ignore others when hasHeaders is true', () => {
            const headers = [
                ...
                'description',
                'DUE DATE',
                'scheduled date', // Test variations for scheduled date
                'DUE DATE TYPE',
                'scheduleddatetype', // Test variations for scheduled date type
                'People',
                'tags',
                'Unknown Column',
            ];
