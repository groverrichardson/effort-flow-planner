...
async function globalSetup() {
  ...
  try {
    ...
    const screenshotPath = (name: string) => path.join(process.cwd(), 'playwright', 'auth-debug', `${name}-${new Date().toISOString().replace(/[:.]/g, '-')}.png`);
    
    // Step 1: Navigate to the login page
    console.log(`Navigating to login page at http://localhost:${process.env.TEST_PORT}/login`);
    await page.goto(`http://localhost:${process.env.TEST_PORT}/login`, { timeout: 30000 });
    await page.screenshot({ path: screenshotPath('01-login-page-loaded'), fullPage: true });
    ...
    try {
      ...
      await Promise.race([
        ...
        page.waitForSelector('[data-testid="login-form"]', { timeout: 10000 }),
        page.waitForSelector('input[type="email"]', { timeout: 10000 })
      ]);
    } catch (error) {
      console.error('Could not find login form. Taking diagnostic screenshot...');
      await page.screenshot({ path: screenshotPath('error-no-login-form'), fullPage: true });
      console.log('Current page HTML:');
      console.log(await page.content());
      throw error;
    }
    
    await page.screenshot({ path: screenshotPath('02-login-form-found'), fullPage: true });
    ...
    try {
      ...
      await page.screenshot({ path: screenshotPath('error-email-input'), fullPage: true });
      throw error;
    }
    ...
    try {
      ...
      await passwordInput.fill(process.env.PLAYWRIGHT_TEST_USER_PASSWORD || '');
    } catch (error) {
      console.error('Failed to fill password:', error);
      await page.screenshot({ path: screenshotPath('error-password-input'), fullPage: true });
      throw error;
    }
    
    await page.screenshot({ path: screenshotPath('03-credentials-filled'), fullPage: true });
    ...
    try {
      ...
      await page.screenshot({ path: screenshotPath('error-login-button'), fullPage: true });
      throw error;
    }
    ...
    try {
      await page.waitForURL(`http://localhost:${process.env.TEST_PORT}/`, { timeout: 30000 });
    } catch (error) {
      console.error('Failed to redirect after login:', error);
      await page.screenshot({ path: screenshotPath('error-redirect-timeout'), fullPage: true });
      throw error;
    }
    
    await page.screenshot({ path: screenshotPath('04-login-successful'), fullPage: true });
    ...
    try {
      ...
      await Promise.race([
        ...
        page.waitForSelector('[data-testid="sidebar"]', { timeout: 5000 })
      ]);
    } catch (error) {
      console.warn('Could not find typical authenticated UI elements. Continuing anyway...');
      await page.screenshot({ path: screenshotPath('warning-no-auth-elements'), fullPage: true });
    ...
    if (fs.existsSync(storageState)) {
      ...
      if (!authData.cookies || authData.cookies.length === 0) {
        ...
        console.log(`Authentication state with ${authData.cookies.length} cookies saved to ${storageState}`);
      }
    } else {
