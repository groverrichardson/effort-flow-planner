...
test.describe('Task Creation Form UI Elements', () => {
    test('should display all form fields with expected labels', async ({
        page,
    }) => {
        console.log(
            'Starting test: should display all form fields with expected labels'
        );
        ...
        await page.screenshot({
            path: 'test-results/task-form-creation-fields.png',
        });
        ...
        const formTitle = await page
            .locator(
                'h1,h2,h3:has-text("Create Task"), [data-testid="task-form-title"]'
            )
            .first();
        ...
        await expect(page.locator('#title, input#title')).toBeVisible({
            timeout: 10000,
        });
        ...
        await expect(
            page.locator('#description, textarea#description')
        ...
        const dueDateField = page
            .locator(
                '#task-form-due-date-trigger, button[id*="due-date-trigger"], [aria-label*="due date"]'
            )
            .first();
        ...
        const priorityField = page
            .locator('#priority, [aria-labelledby="task-form-priority-label"]')
            .first();
        ...
        await expect
            .soft(
                page
                    .locator(
                        'label:has-text("Due Date"), [for*="due_date" i], [data-testid="due-date-label"]'
                    )
                    .first()
            )
        ...
        await expect(
            page
                .locator(
                    'label:has-text("Priority"), [for*="priority" i], [data-testid="priority-label"]'
                )
                .first()
        )
            .toBeVisible({ timeout: 5000 })
            .catch((e) =>
                console.log('Priority field not found with primary selector')
            );
    ...
    test('should have correct priority dropdown options', async ({ page }) => {
        console.log(
            'Starting test: should have correct priority dropdown options'
        );
        ...
        await page.screenshot({
            path: 'test-results/task-form-priority-dropdown-open.png',
        });

        // Find the select options in the dropdown content
        const options = await page
            .locator('[role="option"], [data-radix-select-item]')
            .all();
        ...
        if (options.length > 0) {
            ...
            for (const expectedOption of expectedPriorities) {
                if (
                    ...
                    console.log(
                        `Found expected priority option: ${expectedOption}`
                    );
                } else {
                    console.log(
                        `Expected priority option "${expectedOption}" NOT found`
                    );
                }
            }
    ...
    test('should have correct repeats dropdown options', async ({ page }) => {
        console.log(
            'Starting test: should have correct repeats dropdown options'
        );
        ...
        // Take screenshot after opening dropdown
        await page.screenshot({
            path: 'test-results/task-form-repeats-dropdown-open.png',
        });
        ...
        if (options.length > 0) {
            ...
            for (const expectedOption of expectedOptions) {
                if (
                    ...
                    console.log(
                        `Found expected repeat option: ${expectedOption}`
                    );
                } else {
                    console.log(
                        `Expected option "${expectedOption}" NOT found`
                    );
                }
            }
            ...
            try {
                const weeklyOptionIndex = optionTexts.findIndex((text) =>
                ...
                if (weeklyOptionIndex >= 0) {
                    ...
                    for (const selector of checkboxSelectors) {
                        ...
                        if (checkboxCount > 0) {
                            console.log(
                                `Found ${checkboxCount} checkbox options with selector: ${selector}`
                            );
                            break;
                        }
                    }
                ...
                await page.screenshot({
                    path: 'test-results/task-form-repeat-checkbox.png',
                });
    ...
    test('should have correctly formatted date fields', async ({ page }) => {
        console.log(
            'Starting test: should have correctly formatted date fields'
        );
        ...
        if (dueDateInput) {
            ...
            try {
                ...
                await page.screenshot({
                    path: 'test-results/task-form-date-fields.png',
                });
...
test.describe('Task Detail Page Elements', () => {
    ...
    test.beforeEach(async ({ page }) => {
        if (!testTaskId) {
            ...
            const task = await seedTemplateTask(
                TestTaskTemplate.BASIC,
                'Test Task for Detail View'
            );
    ...
    test('should display task details with correct values', async ({
        page,
    }) => {
        console.log(
            'Starting test: should display task details with correct values'
        );
        ...
        for (const selector of titleSelectors) {
            ...
            if (count > 0) {
                ...
                if (
                    titleText &&
                    titleText.includes('Test Task for Detail View')
                ) {
                    await expect(titleElement).toBeVisible();
                    console.log(
                        `Title found with selector ${selector}: "${titleText}"`
                    );
                    titleFound = true;
                    break;
                }
        ...
        for (const selector of descSelectors) {
            ...
            if (count > 0) {
                ...
                for (const element of elements) {
                    ...
                    console.log(
                        `Potential description with selector ${selector}: "${text}"`
                    );
                    descFound = true;
                }
                if (descFound) break;
        ...
        for (const selector of detailSelectors) {
            ...
            for (const element of elements) {
                ...
                if (text.includes('Priority')) {
                    ...
                    const valueElement = await element
                        .locator('+ span, + div, + p')
                        .first();
    ...
    test('should navigate to edit form with pre-filled values', async ({
        page,
    }) => {
        console.log(
            'Starting test: should navigate to edit form with pre-filled values'
        );
        ...
        if (!editButtonClicked) {
            console.log(
                'Could not find edit button, trying alternative selectors'
            );
...
test.describe('Task Form Validation', () => {
    ...
    test('should validate due date format', async ({ page }) => {
            ...
            if (count > 0) {
                ...
                try {
                    ...
                    const submitButton = page
                        .getByRole('button', {
                            name: /^Create Task$|^Update Task$/i,
                        })
                        .first();
                    ...
                    console.log(
                        'Form submitted, checking for validation errors'
                    );
                        ...
                        if (errorCount > 0) {
                            ...
                            for (let i = 0; i < errorCount; i++) {
                                ...
                                if (text && /date|invalid|format/i.test(text)) {
                                    ...
                                    console.log(
                                        `Date validation error found: ${text}`
                                    );
                                    errorFound = true;
                                    break;
                                }
                    ...
                    if (!errorFound) {
                        console.log(
                            'No specific error message found for date validation'
                        );
                    }
                } catch (e) {
                    console.log(
                        'Could not test invalid date format due to browser validation:',
                        e
                    );
                }
                ...
                await page.screenshot({
                    path: 'test-results/form-validation-date-format.png',
                });
                console.log(
                    'Screenshot taken: form-validation-date-format.png'
                );
        ...
        if (!dueDateFound) {
            console.log(
                'Due date input not found, cannot test date validation'
            );
        }
    });
});
