...
export async function isDialogVisible(page) {
  ...
  const overlayCount = await page.locator('div[data-state="open"][aria-hidden="true"]').count();
  const dialogCount = await page.locator('div[role="dialog"][aria-modal="true"]').count();
  const fixedOverlayCount = await page.locator('.fixed.inset-0').count();
  const modalCount = await page.locator('.modal, [data-modal="true"]').count();
  ...
  return overlayCount > 0 || dialogCount > 0 || fixedOverlayCount > 0 || modalCount > 0;
}

// Function to attempt to close a dialog with multiple strategies and retries
export async function closeDialog(page, options = { maxAttempts: 3, timeout: 1500 }) {
  console.log('[TEST] Dialog detected, attempting to close it');
  
  // Take screenshot before attempting to close
  await page.screenshot({ path: 'screenshots/before-dialog-close-attempt.png' });
  
  // Make multiple attempts to close the dialog
  let closed = false;
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
    console.log(`[TEST] Close dialog attempt ${attempt} of ${options.maxAttempts}`);
    ...
    for (const selector of closeButtons) {
      ...
      if (buttonCount > 0) {
        console.log(`[TEST] Found ${buttonCount} close button(s) with selector: ${selector}`);
        try {
          // Try to get more info about the buttons before clicking
          const buttonInfo = await page.locator(selector).evaluateAll(buttons => {
          ...
          console.log(`[DEBUG] Button details for ${selector}:`, buttonInfo);
          
          await page.locator(selector).first().click({ timeout: 1000, force: true });
          buttonFound = true;
          console.log(`[TEST] Clicked button with selector: ${selector}`);
          break;
        } catch (e) {
          console.log(`[TEST] Failed to click button with selector ${selector}: ${e.message}`);
        }
      }
    }
    ...
    await page.screenshot({ path: `screenshots/after-dialog-close-attempt-${attempt}.png` });
    ...
    console.log(`[TEST] After close attempt ${attempt}, dialog still visible: ${stillVisible}`);
  ...
  if (!closed) {
    ...
    try {
      await page.evaluate(() => {
        ...
        const closeButtons = document.querySelectorAll('button.close, button[aria-label="Close"], [role="dialog"] button');
