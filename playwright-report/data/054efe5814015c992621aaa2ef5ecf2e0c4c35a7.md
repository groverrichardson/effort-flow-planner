...
- Location: /Users/freedommarketing/Desktop/effort-flow-planner/tests/ui.spec.ts:131:5
...
Call log:
  ...
  - 267 pixels (ratio 0.01 of all image pixels) are different.
    ...
    at /Users/freedommarketing/Desktop/effort-flow-planner/tests/ui.spec.ts:136:28
```
...
```ts
   36 |     if (route !== '/login' && page.url().includes('/login')) {
   37 |         console.log(`Navigation to ${route} redirected to login - performing authentication`);
   38 |         await authenticate(page);
   39 |         await page.goto(route);
   40 |         await waitForPageStability(page);
   41 |     }
   42 | }
   43 |
